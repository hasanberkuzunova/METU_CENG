...
bool Flight::addTicket(const Ticket &ticket) {
    /* TODO */
  int i=0,ecount=0,bcount=0;
    int size=tickets.size();
    for(;i<size;size--){
        if(tickets[i].getTicketType()==economy) ecount++;
        else bcount++;
    }
    if(ecount>=economyCapacity && ticket.getTicketType()==economy) return 0;
    if(bcount>=businessCapacity && ticket.getTicketType()==business) return 0;
    
    if(ticket.getTicketType()==economy)  ecount++;
       
    if(ticket.getTicketType()==business) bcount++;
       
    tickets.push_back(ticket);
    
    if(ecount==economyCapacity && bcount==businessCapacity) completed=1;
    return 1;
}
